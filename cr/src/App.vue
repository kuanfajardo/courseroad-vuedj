<template>
  <div id="app">
    ...
    <b-container class="main h-100" fluid>
      <b-row>
        <b-col cols="lg-9 sm-6" :class="{ loading: isLoading, updated: !isLoading}">
        ...
        <b-col cols="lg-3 sm-6">
          <side-bar v-on:deleteSubjects="deleteSelectedSubjects" :selected="selected" :buckets="buckets"></side-bar>
        </b-col>
      </b-row>
    </b-container>
  </div>
        ...
        .then(response => {
          this.$http.get('')
            .then(response => {
              ...
              this.buckets = response.body.buckets
              callback(null, response)
            }, response => {
              // HANDLE ERROR
              var errorType = response.body.error_type
